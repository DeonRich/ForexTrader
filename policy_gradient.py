...
from network_utils import device, np2torch
from policy import CategoricalPolicy, GaussianPolicy
from TradingDqn import Q_Model, testQModel
...
class PolicyGradient(object):
    ...
    def __init__(self, env, config, seed, logger=None):
        ...
        if config.use_baseline:
            ...
            #net = testQModel(1, n_channels, img_height, img_width, self.config.state_history)
            net = Q_Model(1, self.config.sequence_len, self.config.inChannels)
            self.baseline_network = BaselineNetwork(env, config, net)
    ...
    def init_policy(self):
        ...
        #state_shape = list(self.env.observation_space.shape)
        #img_height, img_width, n_channels = state_shape
        #mlp = testQModel(self.env.action_space.n, n_channels, img_height, img_width, self.config.state_history).to(device)
        mlp = Q_Model(self.env.action_space.n, self.config.sequence_len, self.config.inChannels).to(device)
        if self.discrete:
    ...
    def sample_path(self, env, num_episodes = None):
        ...
        while (num_episodes or t < self.config.batch_size):
            ...
            for step in range(self.config.max_ep_len):
                states.append(state)
                action = self.policy.act(states[-1])
            ...
            path = {"observation" : np.concatenate(states),
                    "reward" : np.concatenate(rewards,1),
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            G_t = np.zeros(rewards.shape[0])
            returns = []
            for i in range(rewards.shape[1]-1, -1, -1):
                G_t = rewards[:,i] + self.config.gamma * G_t 
    ...
    def normalize_advantage(self, advantages):
        ...
        mu, sigma = np.mean(advantages), np.std(advantages)
        normalized_advantages = (advantages - mu) / sigma
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def train(self):
        ...
        for t in range(self.config.num_batches):
            ...
            if t % self.config.save_freq == 0:
                torch.save(self.policy.state_dict(), self.config.save_path)
            # compute reward statistics for this batch and log
            avg_reward = np.mean(total_rewards)
            sigma_reward = np.sqrt(np.var(total_rewards) / len(total_rewards))
            msg = "{:,}/{:,} Average reward: {:04.2f} +/- {:04.2f}".format(t+1, self.config.num_batches, avg_reward, sigma_reward)
