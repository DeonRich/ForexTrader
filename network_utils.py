...
def build_mlp(
    ...
    class mlp(nn.Module):
        def __init__(self):
            super().__init__()
            self.inputLayer = nn.Linear(input_size, size)
            self.hiddenLayers = nn.ModuleList([nn.Linear(size, size) for i in range(n_layers)])
            self.ouputLayer = nn.Linear(size, output_size)
        def forward(self, x):
            out = nn.functional.relu(self.inputLayer(x))
            if len(self.hiddenLayers) > 0:
                for layer in self.hiddenLayers:
                    out = nn.functional.relu(layer(out))
            return self.ouputLayer(out)
    return mlp()
    #######################################################
    #########          END YOUR CODE.          ############
